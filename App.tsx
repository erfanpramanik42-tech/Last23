...
const App: React.FC = () => {
  ...
  /**
   * TASK 1: Kalman Filter for GPS Smoothing.
   * Internal algorithm to reduce jitter and improve coordinate accuracy.
   */
  const applyKalmanFilter = (newLat: number, newLng: number, accuracy: number) => {
    ...
    if (state.variance < 0) {
      ...
      const q = 0.000001; // Process noise
   ...
   * Logic: Weight matches based on position (Starts-with > Mid-word > Category match)
   */
  ...
  // Premium Swipe Logic for product detail
  ...
  const handleProductTouchEnd = (e: React.TouchEvent) => {
    ...
    const threshold = window.innerWidth * 0.22; // Snapping threshold
  ...
  /**
   * TASK 4: Smarter Visit Marking Logic.
   * Idempotent logic ensuring race-conditions don't cause duplicate entries.
   * Functional updates ensure current state is used.
   */
  ...
  useEffect(() => { localStorage.setItem('fieldpro_shops', JSON.stringify(shops)); }, [shops]);
  useEffect(() => { localStorage.setItem('fieldpro_areas', JSON.stringify(areas)); }, [areas]);
  useEffect(() => { localStorage.setItem('fieldpro_routes', JSON.stringify(routes)); }, [routes]);
  useEffect(() => { localStorage.setItem('fieldpro_visits', JSON.stringify(visits)); }, [visits]);
  useEffect(() => { localStorage.setItem('fieldpro_products', JSON.stringify(products)); }, [products]);
  useEffect(() => { localStorage.setItem('fieldpro_orders', JSON.stringify(orders)); }, [orders]);
  useEffect(() => { localStorage.setItem('fieldpro_dealers', JSON.stringify(dealers)); }, [dealers]);
  useEffect(() => { localStorage.setItem('fieldpro_range', String(detectionRange)); }, [detectionRange]);
  useEffect(() => { localStorage.setItem('fieldpro_nearby_range', String(nearbyRange)); }, [nearbyRange]);

  useEffect(() => {
    if (!navigator.geolocation) return;
    const watchId = navigator.geolocation.watchPosition(
      (pos) => {
        // Logic: Apply Kalman Filter for smoothing before any other processing
        ...
        // Proximity detection for alerts (independent of current spot list)
        ...
        if (isTrackingRef.current && activeRouteRef.current) {
          /**
           * TASK 2: GPS Displacement Filtering.
           * Filter out GPS noise/jitter. Only record path if displacement > 3m.
           */
  ...
  const toggleTracking = () => {
    if (!isTracking) {
      ...
      // Reset Kalman state for new tracking session
  ...
  const captureCurrentLocation = async () => {
    ...
    } catch (err) { alert("GPS retrieval failed. Ensure you are outdoors with high signal."); }
  };
    ...
    reader.onload = (event) => {
      try {
        ...
        if (!importedData.shops || !importedData.areas || !importedData.routes) {
          throw new Error("Invalid file structure. Required data missing.");
        }
        ...
        const mergedShops = mergeById(shops, importedData.shops);
        const mergedAreas = mergeById(areas, importedData.areas);
        const mergedRoutes = mergeById(routes, importedData.routes);
        const mergedVisits = importedData.visits ? mergeById(visits, importedData.visits) : visits;
        const mergedProducts = importedData.products ? mergeById(products, importedData.products) : products;
        const mergedOrders = importedData.orders ? mergeById(orders, importedData.orders) : orders;
        const mergedDealers = importedData.dealers ? mergeById(dealers, importedData.dealers) : dealers;
        ...
        if (importedData.settings) {
          if (importedData.settings.detectionRange) setDetectionRange(importedData.settings.detectionRange);
          if (importedData.settings.nearbyRange) setNearbyRange(importedData.settings.nearbyRange);
        }
        alert(lang === 'en' ? "Import successful!" : "ইম্পোর্ট সফল হয়েছে!");
      } catch (err: any) {
        alert(lang === 'en' ? `Import failed: ${err.message}` : `ইম্পোর্ট ব্যর্থ হয়েছে: ${err.message}`);
      }
  ...
  const quickAccessItems = [
    ...
    { id: 'reminders', key: 'followUpReminder', icon: <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg> },
    ...
    { id: 'target', key: 'targetProgress', icon: <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg> },
    { id: 'performance', key: 'performance', icon: <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg> },
  ...
  /**
   * TASK 1 & 3: Atomic Order Confirmation with Automated Inventory Deduction and Validation Gate.
   * Logic: 1. Validate stock availability. 2. Deduct inventory. 3. Bind active Dealer info. 4. Save order.
   */
  const confirmOrder = () => {
    if (!orderShop) return;
    if (orderCart.length === 0) return;

    // Snapshot current cart and products for logic validation
    const itemsSnapshot = [...orderCart];
    const totalSnapshot = cartSummary.total;
    const subtotalSnapshot = cartSummary.subtotal;

    // TASK 3: Stock Validation Gate
    const validationErrorItem = itemsSnapshot.find(item => {
    ...
    if (validationErrorItem) {
      alert(lang === 'en' 
        ? `Order Failed: Insufficient stock for ${validationErrorItem.productName}.` 
        : `অর্ডার ব্যর্থ: ${validationErrorItem.productName}-এর পর্যাপ্ত স্টক নেই।`);
      return;
    }

    // TASK 1: Automated Inventory Deduction (Atomic state update)
    setProducts(currentProducts => 
      currentProducts.map(p => {
        const orderedItem = itemsSnapshot.find(item => item.productId === p.id);
        if (orderedItem) {
    ...
    // Logic: Retrieve the first active dealer to bind to the order for reporting
    const activeDealer = dealers.find(d => !d.isArchived);

    // Save Order record with enhanced reporting fields
    const newOrder: Order = {
      id: generateId(),
      shopId: orderShop.id,
      shopName: orderShop.name,
      dealerId: activeDealer?.id,
      dealerName: activeDealer?.dealerName,
  ...
  const deleteOrder = (id: string) => {
    if (window.confirm(lang === 'en' ? 'Delete this order record?' : 'এই অর্ডার রেকর্ডটি মুছে ফেলতে চান?')) {
  ...
  return (
    <div className={`min-h-screen ${isInRideMode ? 'h-screen pb-0' : 'pb-24'} flex flex-col overflow-hidden`}>
      ...
      <main className={`flex-1 flex flex-col ${isInRideMode || viewingRoute || viewingProduct || showOrderSystem || showDealersList || showAnalytics || showSmartRoute ? 'h-full p-0 overflow-hidden' : 'p-4 max-w-4xl mx-auto w-full'} relative`}>
        ...
        {view === 'History' && (
          <div className="h-full flex flex-col relative animate-fadeIn">
            {!viewingRoute ? (
              <div className="space-y-4 h-full overflow-y-auto pb-8 scrollbar-hide">
                <h4 className="font-bold text-slate-700 flex items-center gap-2">
                ...
                {activeRoutes.length > 0 ? activeRoutes.map(route => (
                  <div key={route.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm cursor-pointer hover:border-indigo-200 transition-all active:scale-[0.98]" onClick={() => setViewingRoute(route)}>
                    ...
                    <p className="text-[10px] text-indigo-400 font-black uppercase tracking-widest">{new Date(route.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - {route.endTime ? new Date(route.endTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}</p>
                  </div>
                )) : <div className="py-20 text-center text-slate-400">No history yet.</div>}
              </div>
            ) : null}
          </div>
        )}
          ...
          <div className="flex flex-col h-full gap-6 animate-fadeIn pb-24 overflow-y-auto scrollbar-hide">
            ...
            {isManagingCatalog ? (
              <div className="space-y-6">
                 <div className="flex justify-between items-center bg-indigo-50/50 p-4 rounded-3xl border border-indigo-100">
              ...
              <>
                ...
                <div className="bg-indigo-700 rounded-3xl p-6 text-white shadow-xl space-y-5 relative overflow-hidden">
                  <div className="relative z-10">
                    <h5 className="font-black text-xs uppercase tracking-widest opacity-70 mb-4">Data Management</h5>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-hide">
            {activeDealers.length > 0 ? (
              activeDealers.map(dealer => (
                <div key={dealer.id} className="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm space-y-3 group">
                  <div className="flex justify-between items-start">
      ...
      {/* --- Redesigned Product Detail Presentation View (Swipeable) --- */}
      {viewingProduct && (() => {
        const currentIndex = filteredCatalogProducts.findIndex(p => p.id === viewingProduct.id);
            ...
            >
              {filteredCatalogProducts.map((p) => (
                <div key={p.id} className="w-screen h-full flex flex-col bg-white overflow-hidden relative">
                  ...
                  <div className="flex-[0.2] bg-white px-8 py-6 flex flex-col justify-center border-t border-slate-50 shadow-[0_-10px_30px_-15px_rgba(0,0,0,0.05)]">
                    <div className="flex justify-between items-start gap-4 mb-2">
                      ...
                      <div className="flex flex-col items-end">
                         {p.discount > 0 && (
                           <span className="text-xs font-black text-slate-400 line-through font-bold opacity-70 mb-0.5">৳{p.price}</span>
                         )}
                    ...
                    <div className="flex justify-center mt-4">
                       <div className="flex gap-1.5">
                          {filteredCatalogProducts.map((dot) => (
                            <div 
                              key={dot.id} 
                              className={`h-1 rounded-full transition-all duration-300 ${dot.id === p.id ? 'w-6 bg-indigo-500' : 'w-1 bg-slate-200'}`}
                            />
                          ))}
                       </div>
                    </div>
      ...
      {showOrderSystem && (
        <div className="fixed inset-0 z-[4000] bg-white flex flex-col animate-fadeIn overflow-hidden">
          <header className="bg-white border-b border-slate-100 p-4 flex items-center gap-4 shrink-0 shadow-sm">
            <button 
              onClick={() => { setShowOrderSystem(false); setOrderCart([]); setOrderShop(null); }} 
            ...
            {orderShop && (
              <div className="flex bg-slate-50 p-1 rounded-xl">
                <button 
                  onClick={() => setOrderTab('taking')}
                  className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${orderTab === 'taking' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}
                >
                  Order
                </button>
                <button 
                  onClick={() => setOrderTab('history')}
                  className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${orderTab === 'history' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}
                >
                  Recent
                </button>
              </div>
          ...
          {!orderShop ? (
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2 mb-2">Target Shop</p>
               <div className="space-y-3">
                 {activeShops.map(shop => (
                   <button 
                    key={shop.id} 
                    onClick={() => setOrderShop(shop)}
                    className="w-full bg-slate-50 p-5 rounded-[2rem] border border-transparent hover:border-indigo-100 flex items-center justify-between group active:scale-[0.98] transition-all"
                   >
            ...
            <div className="flex-1 flex flex-col overflow-hidden bg-slate-50/30">
               <div className="bg-white p-4 space-y-3 shrink-0">
                 <div className="relative">
                   <input 
                     type="text" 
                     placeholder="Search products..." 
                     className="w-full bg-slate-50 rounded-2xl py-3 pl-11 pr-4 text-sm font-bold focus:bg-white border-2 border-transparent focus:border-indigo-50 transition-all outline-none"
                     value={orderSearch}
                 ...
                 <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
                    {categories.map(cat => (
                      <button 
                        key={cat}
                        onClick={() => setOrderSearch(cat === 'All' ? '' : cat)}
                        className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${orderSearch === cat ? 'bg-indigo-600 border-indigo-600 text-white shadow-md' : 'bg-white border-slate-100 text-slate-400'}`}
                      >
                        {cat}
                      </button>
                    ))}
                       ...
                       return (
                         <div key={p.id} className="bg-white p-3 rounded-[1.5rem] border border-slate-50 shadow-sm flex items-center gap-3">
                           ...
                           {inCart ? (
                             ...
                             <button onClick={() => addToCart(p)} className="bg-slate-100 text-slate-500 p-2.5 rounded-xl hover:bg-indigo-600 hover:text-white transition-all active:scale-90">
               ...
               {orderCart.length > 0 && (
                 <div className="bg-white border-t border-slate-100 p-6 pb-8 space-y-4 shadow-[0_-20px_40px_-15px_rgba(0,0,0,0.05)] animate-slideUp">
                    ...
                    <button 
                      ...
                      className="w-full bg-indigo-600 text-white font-black py-4 rounded-[1.5rem] shadow-xl shadow-indigo-100 transition-all active:scale-95 uppercase tracking-widest text-xs"
                    >
                      Confirm Order
                    </button>
                 </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/30 scrollbar-hide">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2 mb-2">Orders for {orderShop.name}</p>
                  ...
                  <div key={order.id} className="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
                    ...
                    <div className="flex gap-2">
                       <button 
                        ...
                        className="flex-1 bg-emerald-50 text-emerald-600 font-black text-[9px] uppercase tracking-widest py-2.5 rounded-xl border border-emerald-100 flex items-center justify-center gap-2 active:bg-emerald-100"
                       >
                         <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.246 2.248 3.484 5.232 3.483 8.412-.003 6.557-5.338 11.892-11.893 11.892-1.997-.001-3.951-.5-5.688-1.448l-6.308 1.656zm6.29-4.171c1.515.898 3.3 1.447 5.177 1.448 5.679 0 10.301-4.621 10.303-10.3 0-2.75-1.072-5.335-3.018-7.283-1.946-1.948-4.529-3.02-7.276-3.021-5.684 0-10.306 4.621-10.308 10.3-.001 1.841.488 3.639 1.412 5.201l-.92 3.359 3.435-.904zm10.941-7.145c-.03-.049-.117-.079-.244-.143s-.754-.372-.871-.415-.202-.064-.287.064-.329.415-.403.5-.149.096-.276.032-.537-.198-.989-.602c-.352-.314-.59-.702-.659-.821s-.007-.183.056-.246c.057-.057.127-.148.191-.223s.085-.127.127-.213.021-.16-.011-.224-.287-.69-.393-.948c-.104-.25-.21-.216-.287-.219s-.159-.004-.244-.004-.223.032-.339.159-.446.436-.446 1.063.463 1.233.527 1.318c.064.085.911 1.391 2.206 1.948.308.132.548.211.735.27.31.099.592.085.815.052.249-.037.754-.308.859-.606s.106-.553.074-.606z"/></svg>
                         WhatsApp
                       </button>
                       <button 
                        onClick={() => deleteOrder(order.id)}
                        className="px-3 py-2.5 rounded-xl text-rose-300 hover:text-rose-500 active:bg-rose-50"
                       >
        ...
        <div className="fixed inset-0 z-[1000] animate-fadeIn">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-[2.5rem] shadow-[0_-20px_50px_-15px_rgba(0,0,0,0.35)] p-6 pb-12 animate-slideUp max-h-[85vh] overflow-y-auto scrollbar-hide">
            <div className="flex justify-between items-center mb-6 px-2">
              ...
              <button onClick={() => setShowQuickAccess(false)} className="p-2.5 bg-slate-50 text-slate-400 hover:text-rose-500 rounded-full transition-all active:scale-90 border border-slate-100">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 px-2">
              {quickAccessItems.map(item => (
                <button 
                  key={item.id}
                  className="flex flex-col items-center gap-3 p-4 rounded-[2rem] bg-white border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50/10 transition-all active:scale-95 group shadow-sm"
                  onClick={() => {
                    if (item.id === 'catalog') {
                      setShowCatalog(true);
                ...
                >
                  <div className="w-16 h-16 bg-slate-50 rounded-[1.5rem] flex items-center justify-center text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-300">
                    {item.icon}
                  </div>
                  <span className="text-[10px] font-black text-slate-700 uppercase tracking-widest text-center leading-tight">
                    {t(item.key)}
                  </span>
                </button>
              ))}
          ...
          <div className="bg-white w-full max-md rounded-[2.5rem] overflow-hidden shadow-2xl animate-scaleUp my-auto">
            ...
            <form onSubmit={saveProduct} className="p-6 space-y-5">
               <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100 space-y-4">
                  <div className="flex items-center gap-4 border-b border-slate-200 pb-4">
                    ...
                    <button type="button" onClick={() => productPhotoRef.current?.click()} className="w-16 h-16 bg-white rounded-2xl shadow-sm border-2 border-dashed border-indigo-200 flex items-center justify-center text-indigo-600 transition-all active:scale-95 overflow-hidden">
                       {editingProduct?.photo ? (
                         <img src={editingProduct.photo} className="w-full h-full object-cover" />
                       ) : (
                         <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                       )}
                    </button>
                    <div>
                      <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Product Image</p>
                      <button type="button" onClick={() => productPhotoRef.current?.click()} className="text-[11px] font-bold text-slate-500 underline decoration-slate-300">Tap to upload photo</button>
                    </div>
                  </div>

                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{t('productName')}*</label>
                    <input required type="text" placeholder="e.g. Master Soap Bar" className="w-full bg-white rounded-xl px-4 py-3 border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-sm transition-all" value={editingProduct?.name || ''} onChange={e => setEditingProduct(prev => ({ ...prev, name: e.target.value }))} />
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Category*</label>
                      <input required type="text" placeholder="e.g. Hygiene" className="w-full bg-white rounded-xl px-4 py-3 border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-sm" value={editingProduct?.category || ''} onChange={e => setEditingProduct(prev => ({ ...prev, category: e.target.value }))} />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Weight/Unit</label>
                      <input type="text" placeholder="e.g. 150g" className="w-full bg-white rounded-xl px-4 py-3 border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-sm" value={editingProduct?.weight || ''} onChange={e => setEditingProduct(prev => ({ ...prev, weight: e.target.value }))} />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <input required type="number" placeholder="0.00" className="w-full bg-white rounded-xl px-4 py-3 border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-sm" value={editingProduct?.price || ''} onChange={e => setEditingProduct(prev => ({ ...prev, price: Number(e.target.value) }))} />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Discount (%)</label>
                      <input type="number" placeholder="0" className="w-full bg-white rounded-xl px-4 py-3 border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-sm" value={editingProduct?.discount || ''} onChange={e => setEditingProduct(prev => ({ ...prev, discount: Number(e.target.value) }))} />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <input type="number" placeholder="0" className="w-full bg-white rounded-xl px-4 py-3 border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-sm" value={editingProduct?.stock || ''} onChange={e => setEditingProduct(prev => ({ ...prev, stock: Number(e.target.value) }))} />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Status</label>
                      <button type="button" onClick={() => setEditingProduct(prev => ({ ...prev, status: prev?.status === 'Active' ? 'Inactive' : 'Active' }))} className={`w-full py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${editingProduct?.status === 'Active' ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : 'bg-slate-200 border-slate-300 text-slate-500'}`}>
               ...
               {editingProduct?.price && (editingProduct?.discount || 0) > 0 && (
                  <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100 flex justify-between items-center px-6">
                     <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Final Sales Price</span>
                     <span className="text-sm font-black text-indigo-700">৳{calculateFinalPrice(Number(editingProduct.price), Number(editingProduct.discount || 0))}</span>
                  </div>
               )}
        ...
        <div className="fixed inset-0 z-[800] bg-slate-900/70 backdrop-blur-md p-4 flex justify-center items-center overflow-y-auto">
          <div className="bg-white w-full max-md rounded-3xl overflow-hidden shadow-2xl animate-scaleUp my-auto">
            <div className="p-6 bg-emerald-600 text-white flex justify-between items-center">
              ...
              <button onClick={() => { setIsSavingRoute(false); setActiveRoute(null); }} className="transition-all active:scale-90">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            <form onSubmit={confirmSaveRoute} className="p-6 space-y-5">
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex justify-between items-center">
                <div className="flex flex-col">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Automatic Date</span>
                  <span className="font-bold text-slate-700">{new Date().toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })}</span>
                </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Day of Week</label>
                  <input required type="text" placeholder="e.g. Saturday" className="w-full bg-slate-50 rounded-2xl px-5 py-4 border border-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none font-bold" value={routeSaveForm.day} onChange={e => setRouteSaveForm(prev => ({ ...prev, day: e.target.value }))} />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Trip / Area Name</label>
                  <input required type="text" placeholder="e.g. Uttara Sec-4 Trip" className="w-full bg-slate-50 rounded-2xl px-5 py-4 border border-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none font-bold" value={routeSaveForm.areaName} onChange={e => setRouteSaveForm(prev => ({ ...prev, areaName: e.target.value }))} />
                </div>
              </div>
              <div className="flex gap-3 pt-2">
            ...
            <div className="p-6 space-y-6">
              <form onSubmit={addArea} className="space-y-3">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <select className="w-full bg-slate-50 rounded-xl px-4 py-2 text-xs font-bold border border-slate-200" value={newAreaDay} onChange={e => setNewAreaDay(e.target.value)}>
                  ...
                  {activeAreas.map(area => (
                    <div key={area.id} className="flex flex-col gap-1 p-3 bg-slate-50 border border-slate-100 rounded-xl">
                      <div className="flex items-center justify-between">
                        <input type="text" className="bg-transparent font-bold text-slate-700 flex-1 outline-none focus:bg-white px-1 rounded" value={area.name} onChange={(e) => updateAreaDetails(area.id, e.target.value, area.assignedDay || '')} />
                      ...
                      <select className="bg-transparent text-[10px] font-black uppercase text-indigo-500 outline-none w-fit" value={area.assignedDay} onChange={(e) => updateAreaDetails(area.id, area.name, e.target.value)}>
                        {WEEKDAYS.map(day => <option key={day} value={day}>{day}</option>)}
                      </select>
                    </div>
                  ))}
      ...
      {viewingShop && (
        <div className="fixed inset-0 z-[500] bg-slate-900/80 backdrop-blur-sm p-4 flex items-center justify-center overflow-y-auto">
          <div className="bg-white w-full max-w-lg rounded-[3.5rem] overflow-hidden shadow-2xl animate-scaleUp my-auto relative">
            ...
            <div className="h-64 w-full bg-slate-100 relative overflow-hidden">
              ...
              <div className="absolute bottom-16 left-8 flex items-center gap-2">
                <span className="bg-indigo-600 text-white text-[10px] font-black px-3 py-1 rounded-full border border-indigo-400/50 uppercase tracking-widest shadow-lg">
                  {activeAreas.find(a => a.id === viewingShop.areaId)?.name}
                </span>
                {viewingShop.subArea && (
                ...
                <span className="bg-white/20 backdrop-blur-md text-white text-[10px] font-bold px-3 py-1 rounded-full border border-white/30 uppercase tracking-widest">Verified</span>
              </div>
            </div>
            <div className="bg-white px-8 pb-10 pt-16 -mt-12 rounded-t-[3.5rem] relative z-10 shadow-[0_-35px_70px_-15px_rgba(0,0,0,0.35)]">
              <div className="flex justify-between items-start mb-6">
                <div className="flex-1 min-w-0 pr-4">
                  <div className="flex items-center gap-3">
                    <h2 className="text-3xl font-black text-slate-900 leading-tight mb-1">{viewingShop.name}</h2>
                    {isVisitedToday(viewingShop.id) && <span className="bg-emerald-500 text-white rounded-full p-1.5 shadow-lg animate-scaleUp"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M5 13l4 4L19 7" /></svg></span>}
                  </div>
              ...
              <div className="space-y-5">
                 ...
                 {viewingShop.subArea && (
                   <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex flex-col"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">{t('subArea')}</p><p className="text-lg font-bold text-slate-700 leading-tight truncate">{viewingShop.subArea}</p></div>
                 )}
                 
                 <div className="flex gap-3">
                   <button 
                     onClick={() => toggleVisit(viewingShop.id)}
                     className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-2xl font-black text-xs uppercase tracking-widest transition-all border shadow-md active:scale-95 ${isVisitedToday(viewingShop.id) ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : 'bg-slate-900 border-slate-800 text-white'}`}
                   >
                     {isVisitedToday(viewingShop.id) ? (
                       <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M6 18L18 6M6 6l12 12" /></svg> {t('unmarkVisited')}</>
                     ) : (
                       <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg> {t('markVisited')}</>
                     )}
                   </button>
                   <button 
                     onClick={() => {
                       setOrderShop(viewingShop);
                       setOrderTab('taking');
                       setShowOrderSystem(true);
                       setViewingShop(null);
                     }}
                     className="flex-1 py-3.5 bg-white border border-slate-200 text-slate-600 font-black text-xs uppercase rounded-2xl shadow-sm active:scale-95 transition-all"
                   >
                     Take Order
                   </button>
                 </div>
              ...
              <button onClick={() => startNavigation(viewingShop)} className="w-full mt-8 bg-indigo-600 text-white font-black py-5 rounded-[1.8rem] shadow-2xl shadow-indigo-200 flex items-center justify-center gap-3 transition-all active:scale-95 hover:bg-indigo-700">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M17.657 16.657L13.414 20.9m0 0l-1.414-1.414m1.414 1.414L15.828 18.07M12 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                {t('getDirections')}
              </button>
            </div>
          </div>
      ...
      {isEditingShop && (
        <div className="fixed inset-0 z-[600] bg-slate-900/60 backdrop-blur-sm p-4 flex justify-center overflow-y-auto items-start md:items-center">
          <div className="bg-white w-full max-md rounded-3xl overflow-hidden shadow-2xl animate-scaleUp my-4 md:my-8">
            ...
            <form onSubmit={saveShop} className="p-6 space-y-4">
              ...
              <div className="bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100">
                <div className="flex justify-between items-center mb-3">
                  <label className="block text-xs font-black text-indigo-900 uppercase">{t('map')} Location</label>
                  <button type="button" onClick={captureCurrentLocation} className="bg-indigo-600 text-white text-[10px] font-bold px-3 py-1.5 rounded-full transition-all active:scale-95">Current GPS</button>
                </div>
                {editingShop?.location && (
                  <p className="text-[10px] text-indigo-400 font-mono mb-2">Lat: {editingShop.location.lat.toFixed(6)}, Lng: {editingShop.location.lng.toFixed(6)}</p>
                )}
                <LocationPickerMap 
              ...
              <div>
                <label className="block text-xs font-black text-slate-500 uppercase mb-1">{t('photo')}</label>
                <div className="flex items-center gap-4">
                  <input type="file" accept="image/*" className="hidden" id="photo-upload" onChange={handlePhotoUpload} />
                  <label htmlFor="photo-upload" className="bg-indigo-50 text-indigo-700 px-5 py-2.5 rounded-xl text-sm font-black border border-indigo-100 cursor-pointer transition-all active:scale-95 hover:bg-indigo-100 uppercase tracking-widest">Capture Photo</label>
                  {editingShop?.photo && <div className="flex items-center gap-2"><div className="w-10 h-10 rounded-lg overflow-hidden border border-indigo-200"><img src={editingShop.photo} className="w-full h-full object-cover" /></div><span className="text-[10px] text-emerald-500 font-black uppercase">✓ OK</span></div>}
                </div>
              </div>
