...
export const MapComponent: React.FC<MapComponentProps> = ({ 
  ...
  const handleDragStart = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    dragStart.current = { x: clientX - capsuleOffset.x, y: clientY - dragStart.current.y };
    e.stopPropagation();
  };
  ...
  const toggleFollow = () => {
    ...
    if (!isFollowing && currentLocation && leafletMap.current) {
      leafletMap.current.panTo([currentLocation.lat, currentLocation.lng], { animate: true });
    }
  };
  ...
  useEffect(() => {
    ...
    if (!leafletMap.current) {
      leafletMap.current = L.map(mapRef.current, {
        ...
        preferCanvas: true,
        rotate: true, // Enable rotation via plugin
        touchRotate: true // Plugin option for mobile
      }).setView([23.8103, 90.4125], 13);

      layersRef.current.street = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(leafletMap.current);
      ...
      const onTouchStart = (e: TouchEvent) => {
        if (e.touches.length === 2) {
          ...
          setIsHeadingUp(false); // Disable heading tracking if manually rotating
        }
      };
      ...
      // Increased timeout to ensure layout/animation is complete
      setTimeout(() => {
  ...
  // Ensure map visibility when activeRoute changes (important for history view)
  useEffect(() => {
  ...
  // Shop Markers Effect - Modified to hide when viewing history for a "minimal" look
  useEffect(() => {
    ...
    // HIDE SHOPS IF VIEWING HISTORY (Minimal Design)
  ...
  useEffect(() => {
    ...
    if (!userMarkerRef.current) {
      userMarkerRef.current = L.marker([currentLocation.lat, currentLocation.lng], {
        zIndexOffset: 3000, // Very high to stay above polylines and stop markers
        icon: L.divIcon({
      ...
      userMarkerRef.current.setZIndexOffset(3000);
    }
    ...
    if (markerElement) {
      ...
      if (container) {
        if (isHeadingUp) {
          // If map rotates with heading, arrow should point fixed 'Up' relative to map
          leafletMap.current.setBearing(heading);
          container.style.transform = `rotate(0deg)`;
        } else {
          // Otherwise arrow rotates freely on north-up map
  ...
  // Unified Route & Timeline Rendering (Minimal & Clear)
  useEffect(() => {
    if (!leafletMap.current || !activeRoute) {
    ...
    // Draw Polyline (Clean and focused)
    ...
    // Draw Stops (Timeline Points) - Minimalist dots
    stopMarkersRef.current.forEach(m => m.remove());
    stopMarkersRef.current = (activeRoute.stops || []).map((stop, idx) => {
      ...
      // Timeline labels - Minimal and non-distracting
      if (isHistoryView) {
        marker.bindTooltip(`
    ...
    // Fit view for history with generous padding
    if (isHistoryView && routeLineRef.current.getLatLngs().length > 0) {
      setIsFollowing(false);
      // Extra check to ensure map dimensions are valid before fitBounds
      leafletMap.current.invalidateSize();
      leafletMap.current.fitBounds(routeLineRef.current.getBounds(), { padding: [80, 80] });
    }
  }, [activeRoute?.id, !!activeRoute?.endTime]);

  // Navigation Routing Effect with safety checks
  useEffect(() => {
    ...
    const fetchRoute = async () => {
      try {
        ...
        // Check if map and target are still available after async fetch
        ...
        if (data.routes && data.routes[0]) {
          ...
          if (leafletMap.current) {
            ...
            leafletMap.current.fitBounds(roadNavLineRef.current.getBounds(), { padding: [50, 50] });
          }
  ...
  const getDirectionIcon = (modifier: string) => {
    ...
    switch (modifier?.toLowerCase()) {
      ...
      case 'uturn':
        return <svg className={base} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 4v10a4 4 0 004 4h8a4 4 0 004-4V4M4 4l3 3M4 4L1 7" /></svg>;
      default:
  ...
  return (
    <div className="relative w-full h-full min-h-[300px] overflow-hidden flex-1">
        ...
        <div className="absolute top-6 left-1/2 -translate-x-1/2 z-20 w-[85%] max-w-sm pointer-events-auto" style={{ transform: `translate(calc(-50% + ${capsuleOffset.x}px), ${capsuleOffset.y}px)` }} onMouseDown={handleDragStart} onTouchStart={handleDragStart}>
          <div className="bg-slate-900/85 backdrop-blur-md text-white rounded-2xl p-2.5 shadow-2xl border border-white/10 flex items-center gap-3 animate-fadeIn select-none">
            ...
            <div className="flex-1 min-w-0">
               ...
               {navSteps.length > 0 && <div className="flex items-center gap-2 mt-0.5"><span className="text-[10px] font-black text-white/70">{Math.round(navSteps[currentStepIndex].distance)}m</span><div className="flex-1 h-0.5 bg-white/10 rounded-full overflow-hidden"><div className="h-full bg-indigo-500 w-1/4"></div></div></div>}
            </div>
      ...
      <style>{`
        ...
        /* Pulsing animation for active user icon when tracking on polyline */
        .active-ping::after {
          content: '';
          position: absolute;
          width: 20px;
